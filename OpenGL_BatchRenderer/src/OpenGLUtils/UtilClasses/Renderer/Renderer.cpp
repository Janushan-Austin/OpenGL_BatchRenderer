...
std::unordered_map<MaterialID, Renderer::MaterialMapStruct> Renderer::MaterialMap{};
glm::mat4 Renderer::ProjectionMatrix{ 1.0f }, Renderer::ViewMatrix{ 1.0f };
uint32_t Renderer::InstancingVAO{ 0 }, Renderer::InstancingVBO{ 0 };
uint32_t Renderer::MaxInstances{ 1000000 };
...
void Renderer::Draw(std::vector<Mesh>& meshesToRender, std::vector<std::vector<glm::mat4>>& transforms)
{
...
void Renderer::PutInRenderingBucket(Material* material, MaterialID materialID, Mesh* mesh, MeshID meshID, std::vector<glm::mat4>& transforms)
{
